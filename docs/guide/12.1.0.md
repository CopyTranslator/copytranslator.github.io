...
- [AI 翻译服务列表](#ai-翻译服务列表)：内置与可配置的 AI 服务一览
- [AI 翻译供应商设置](#ai-翻译供应商设置)：配置与管理翻译供应商
- [翻译引擎与词典扩展](#翻译引擎与词典扩展)：新增多家翻译引擎与 Bing 词典
- [翻译器管理](#翻译器管理)：分组配置、配置校验与配置指南
- [网络代理](#网络代理)：HTTP/SOCKS5 代理与离线语言检测
- [内容区域样式](#内容区域样式)：内边距与行高设置
- [OCR 配置](#ocr-配置光学字符识别)：新增 OCR 配置入口
- [便携模式](#便携模式)：随身携带配置
- [使用技巧提示](#使用技巧提示)：不再显示使用技巧
:::
...
在设置中可以调整**内容内边距**与**行高**，优化对照与专注模式的阅读体验。
...
v12.1.0 新增阿里云、Azure、DeepL、腾讯云、腾讯交互、Yandex、火山翻译引擎，并补充彩云翻译与 Bing 词典引擎。

### 翻译器管理

设置中的翻译器管理页面已改版为表格布局，支持分组配置与拖拽排序（缓存/对比/双击分组），并提供更细化的配置校验提示与配置指南链接。

### 网络代理

在设置中新增网络代理页面，支持 HTTP/SOCKS5 代理，可用于解决网络不稳定或访问受限问题。使用代理访问谷歌翻译的具体步骤请参考[常见问题](/guide/questions.html#谷歌翻译退出中国市场的解决方案)。
...
- 翻译引擎: Google, Baidu, Caiyun, Tencent, Tencent Smart, Aliyun, Azure, DeepL, Yandex, Volcengine, Bing, Youdao, Sogou, StepFun, OpenAI 等
- 查词引擎: Google, Bing
:::
...
当可执行文件所在目录存在 `copytranslator` 文件夹时，程序会优先使用该目录作为配置存储位置，便于随身携带配置。
...
设置中新增“不再显示使用技巧”选项，可关闭启动时或特定场景的提示弹窗。
...
v12.1.0 新增 OCR 配置入口与开关，配置路径为设置 → OCR。
